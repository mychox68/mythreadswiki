# RAG
> 관련 노트: 11개 | 마지막 갱신: 2026-07-29 17:24

## 개요

주요 키워드: **RAG** · **에이전트** · **데이터** · **문서** · **Claude** · **개발** · **설계** · **AI가**
...
## Agent Insight
- 시간순으로 3월 말(OpenRAG, LangGraphics 등 완성형 플랫폼 소개) → 4월 초(카파시의 개인 위키, Claude Code 문서 설계) → 4월 중순(Vectorless, 책갈피식 검색) → 4월 말(TurboQuant 압축) 순서를 보면, "무거운 벡터 인프라 구축"에서 "가볍고 파일 기반인 대안 모색"으로 무게중심이 이동한다.
- RAG 개념이 벡터DB에 국한되지 않고, Claude Code에 회사 문서를 사전에 읽혀 설계 방향을 잡게 하는 방식(lian.lab71)이나 카파시의 "LLM이 원시 데이터로 위키를 자동 생성"하는 방식처럼, 컨텍스트를 미리 구조화해 두는 넓은 의미로 계속 재정의되고 있다.
- 효율화 축에서 상반된 두 흐름이 공존한다: 엔터프라이즈는 인덱스 압축(TurboQuant 31GB→4GB)과 멀티모달 통합(Gemini Embedding 2)으로 "더 정교한 벡터"를 추구하는 반면, 에이전트 개발자 쪽은 Vectorless·OpenViking처럼 "벡터 없이 파일시스템처럼" 단순화하는 방향으로 갈라진다.
- 실행 시사점: Claude Code 기반 리서치·설계 작업에서는 벡터DB부터 세팅하기보다, 회사 목표·제약을 담은 문서를 먼저 읽히고 랭킹·검색 스킬을 정의하는 lian.lab71식 "문서 사전 로딩" 패턴이 더 적은 인프라로 RAG의 효과를 낸다.
