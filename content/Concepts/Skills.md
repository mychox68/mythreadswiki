# Skills
> 관련 노트: 8개 | 마지막 갱신: 2026-07-20 13:48

## 개요

주요 키워드: **Claude** · **Code** · **Skills** · **AI** · **해커톤** · **설정** · **Hooks** · **MCP**
...
- Claude Code Skills가 대폭 증가(+598%)하여 실무 직결 스킬이 추가되었다. awesome-claude-skills, alirezarezvani/claude-ski…

## Agent Insight
- 같은 날(3/27)에 "앤트로픽 해커톤 우승자" 가이드를 다루는 노트가 3개나 중복 등장한 것은 이 콘텐츠가 커뮤니티에서 빠르게 확산·재게시되었음을 보여주는 신호다 — Skills/Hooks/MCP 조합이 그만큼 반향을 일으킨 주제였다는 뜻.
- "Skill"이라는 패키징 개념이 Claude Code(Anthropic)에서 출발해 OpenAI(Skills+Shell 워크플로우)를 거쳐 4월에는 구글 크롬 브라우저 기능으로까지 확장됐다 — 특정 도구의 기능이 아니라 AI 상호작용의 범용 패턴으로 표준화되는 궤적.
- Skills 생태계가 +598% 성장하고 65,000개 이상의 스킬이 색인되는 규모에 이르면서, 오히려 "무엇을 골라 쓸 것인가"라는 발견(discovery) 문제가 새로 생겼다 — 큐레이션 사이트·가이드북 제작이 성장과 동시에 등장한 것도 이 때문.
- 3/30 노트(Opus 4.6 + Remotion)는 Skills가 단순 프롬프트 저장을 넘어 영상 렌더링처럼 복잡한 멀티스텝 워크플로우를 캡슐화하는 방향으로 진화하고 있음을 보여준다.
- 실행 시사점: 스킬을 하나하나 검색하기보다 이미 검증된 리포(awesome-claude-skills, alirezarezvani/claude-skills)와 `npx skills add` 같은 설치 표준을 우선 채택해 개인 스킬 라이브러리를 구축하는 편이 65,000개 옵션의 발견 비용을 줄인다.

## 관련 개념

[[Concepts/Claude-Code|Claude Code]] | [[Concepts/AI-코딩|AI 코딩]] | [[Concepts/github.com|github.com]] | [[Concepts/Anthropic|Anthropic]] | [[Concepts/Manus|Manus]]
