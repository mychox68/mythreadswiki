# GitHub
> 관련 노트: 36개 | 마지막 갱신: 2026-09-08 20:46
...
- "유출 → 커뮤니티 재구현 → 대규모 스타"패턴이 반복된다(claw-code가 유출 몇 시간 만에 6만 스타, PM Skills Marketplace가 72시간 만에 1,300 스타) — 이는 04-03 노트의 "기업 보안보다 집단지성 속도가 빠른 시대"라는 명시적 주장과 일치한다.
- clawsweeper 사례(Codex AI 50개를 병렬 투입해 이슈 4,000개를 하루에 정리)는 오픈소스 메인테이너의 반복 노동 자체가 자동화 대상이 되는 단계로 진입했음을 보여준다.
- Spec Kit·Spec-Driven Development 노트들은 "코딩 스킬보다 프로덕트 감각·문서화 능력이 중요해진다"는 언급과 함께 등장해, 개발자 역량의 무게중심이 구현에서 스펙 설계로 이동하는 트레이드오프를 드러낸다.
- 실행 시사점: 코드 작성 자동화 도구를 늘리기보다, 지금 GitHub 트렌딩을 반복 지배하는 "코드베이스 이해/스펙 문서화" 계열 도구(Understand-Anything, spec-kit)를 먼저 파이프라인에 넣는 우선순위가 데이터로 뒷받침된다.
